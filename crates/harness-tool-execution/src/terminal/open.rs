...
use super::{Manager, OPEN_NAME, failure, manager, output};
...
impl ToolExecutor for OpenExecutor {
    fn execute(
        ...
        Box::pin(async move {
            let result = tokio::task::spawn_blocking(move || manager.open(&workspace, &input))
                .await
                .map_err(|e| ToolFailure::Execution(e.to_string()))?;
            match result {
                Ok(text) => Ok(output(text, OPEN_NAME)),
...
pub fn registration(workspace: WorkspaceRoot) -> Arc<dyn ToolExecutor> {
    Arc::new(OpenExecutor::new(workspace))
}
