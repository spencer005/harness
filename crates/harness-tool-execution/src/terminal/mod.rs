...
fn format_output(id: i32, output: &Arc<Mutex<Vec<u8>>>, input: Option<&str>, exit: Option<u32>) -> String { let bytes = std::mem::take(&mut *output.lock().expect("terminal output lock")); let mut text = String::from_utf8_lossy(&bytes).into_owned(); if let Some(input) = input { text.push_str(&format!("echoed input: {input}")); } let mut result = format!("terminal: {id}\n{text}"); if let Some(exit) = exit { result.push_str(&format!("exit code: {exit}\n")); } result }
